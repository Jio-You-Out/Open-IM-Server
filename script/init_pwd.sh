...
sed -i "/^\([[:space:]]*dbMysqlPassword: *\)\".*/s//\1$MYSQL_PASSWORD/;\"/\([[:space:]]*dbPassword: *\)\".*/s//\1$MONGO_PASSWORD/;\"/\([[:space:]]*secret: *\)\".*/s//\1$PASSWORD/;\"/\([[:space:]]*secretAccessKey: *\)\".*/s//\1$MINIO_SECRET/;\"" ../config/usualConfig.yaml

sed -i "/\([[:space:]]*endpoint: *\).*/s##\1$MINIO_ENDPOINT#;" ../config/usualConfig.yaml
sed -i "/\([[:space:]]*dbPassWord: *\)\".*/s//\1$REDIS_PASSWORD/;\"" ../config/usualConfig.yaml
